...
static void _swap16(unsigned char *ptr)
{
...
static void _swap32(unsigned char *ptr)
{
	register unsigned char val;
...
static void _swap64(unsigned char *ptr)
{
	register unsigned char val;
...
bool FileRead :: getWavInfo( const char *fileName )
{
  ...
  while ( strncmp(id, "fmt ", 4) ) {
    ...
    _swap32((unsigned char *)&chunkSize);
#endif
...
#ifndef __LITTLE_ENDIAN__
  _swap16((unsigned char *)&format_tag);
  _swap32((unsigned char *)&chunkSize);
#endif
...
#ifdef __LITTLE_ENDIAN__
  _swap32((unsigned char *)&dataOffset_);
#endif

  // Get length of data from the header.
  if ( fread(&fileSize_, 4, 1, fd_) != 1 ) goto error;
#ifdef __LITTLE_ENDIAN__
  _swap32((unsigned char *)&fileSize_);
#endif
...
#ifdef __LITTLE_ENDIAN__
  _swap32((unsigned char *)&mantissa);
#endif
  exp = 30 - *(srate+1);
  last = 0;
  while (exp--) {
...
bool FileRead :: getMatInfo( const char *fileName )
{
  ...
  if ( byteswap_ ) _swap32((unsigned char *)&datatype);
  if (datatype != 14) {
  ...
  if (byteswap_) _swap32((unsigned char *)&tmp);
  ...
  if (byteswap_) _swap32((unsigned char *)&rows);

  // Get number of columns from the header.
  SINT32 columns;
  if ( fread(&columns, 4, 1, fd_) != 1 ) goto error;
  if (byteswap_) _swap32((unsigned char *)&columns);
  ...
  if (byteswap_) _swap32((unsigned char *)&headsize);
